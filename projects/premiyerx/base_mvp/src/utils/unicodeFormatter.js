...
export function applyFormatToSelection(text, selStart, selEnd, formatter) {
  if (selStart === selEnd) return { text, cursor: selEnd }
  const selected = text.slice(selStart, selEnd)
  const formatted = formatter(selected)
  const newText = text.slice(0, selStart) + formatted + text.slice(selEnd)
  return { text: newText, cursor: selStart + formatted.length }
}
